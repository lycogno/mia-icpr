    ...
    for prune_sparsity in sparsity:
        for num_epochs in epoch_count:
            ...
            for shadow_ind in range(args.shadow_num):
                ...
                pruned_shadow_model_save_folder = f"shadow_models_afterprune/shadow_{shadow_ind}{pruned_model_filename}"
                print(f"Load Pruned Shadow Model From shadow_models_afterprune/shadow_{shadow_ind}{pruned_model_filename}")
                model = CIFAR10Module(args)
                # for module_name, module in  model.model.named_modules():
                #     try:
                #         prune.identity(module, name="weight")
                ...
                #     except:
                #         pass
                model.load_state_dict(torch.load(pruned_shadow_model_save_folder))
