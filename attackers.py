...
def predict_target_sensitivity(self, data_loader, m=10, epsilon=1e-3):
    ...
    with torch.no_grad():
        for inputs, targets in data_loader:
            ...
            new_predicts = F.softmax(self(evaluation_points), dim=-1)
    ...
    def _prepare(self):
        ...
        for shadow_model, shadow_pruned_model, shadow_train_loader, shadow_test_loader in zip(
            ...
            if self.attack_original:
                ...
                attack_in_predicts, attack_in_targets, attack_in_sens = \
                    predict_target_sensitivity(shadow_pruned_model, shadow_train_loader)
                attack_out_predicts, attack_out_targets, attack_out_sens = \
                    predict_target_sensitivity(shadow_pruned_model, shadow_test_loader)
        ...
        if self.attack_original:
            ...
            try:
                ...
                self.victim_in_predicts, self.victim_in_targets, self.victim_in_sens = \
                    predict_target_sensitivity(self.victim_pruned_model, self.victim_train_loader)
                self.victim_out_predicts, self.victim_out_targets, self.victim_out_sens = \
                    predict_target_sensitivity(self.victim_pruned_model, self.victim_test_loader)
