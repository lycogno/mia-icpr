...
def iterative_pruning_finetuning(model,
                                 ...
                                 shadow_model_dir="shadow_models_afterprune",
                                 pruned_model_filename="pruned_model.pt"):
    ...
    for i in range(num_iterations):
        ...
        # for module_name, module in model.named_modules():
        #     ## Hack for resnet18, densenet121 and mobilenet_v2
        #     if "model.classifier" in module_name or "model.fc" in module_name:
        #         try:
        #             prune.remove(module, "weight")
        #         except:
        #             pass
        # model = load_model(model=model,
        #                    model_filepath=f"{shadow_model_dir}/shadow_0{model_filename}",
        #                    device=device)
...
def main():
    ...
    model_filename = args.classifier+"_cifar10.pt"
    model_filename_prefix = args.classifier+"_pruned_model"
    pruned_model_filename = args.classifier+"_afterprune.pt"
    model_filepath = os.path.join(model_dir, model_filename)
        ...
        for num_epochs in epoch_count:
            for i in range(5):
                ...
                pruned_model = iterative_pruning_finetuning(
                    ...
                    model_dir=model_dir,
                    grouped_pruning=False,
                    pruned_model_filename=pruned_model_filename,)

                # Apply mask to the parameters and remove the mask.
                # _= pruned_model(torch.random(1, 3, 32, 32).to(cuda_device))
                ...
                save_model(model=model,
                        ...
                        model_filename="shadow_"+str(i)+pruned_model_filename)
                print("Saved shadow_"+str(i)+pruned_model_filename)
